...
export function AgentCard(props: { agent: AgentSummary }) {
  const { data: feedback, isLoading: isFeedbackLoading } =
    useErc8004AgentFeedback(props.agent.agentId);
  ...
  const explorerLink = `${erc8004Config.explorer}/${props.agent.agentId.split(":").pop()}`;
